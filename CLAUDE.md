...
- Any code writing/review/refactor (dashboard, bot, mod — near-universal) → apply `andrej-karpathy-skills:karpathy-guidelines` before proposing the change, not just when explicitly asked
...
- Pre-merge security pass beyond mineflayer path handling (auth, cookies, MCFarmManager token) → `security-review`
- Library/API doc lookup (Fastify, React Query, mineflayer, zod, better-sqlite3) → `context7` MCP instead of relying on training data
- Multi-step reasoning on a hard bug or design tradeoff → `sequential-thinking` MCP
- Symbol-level code nav (find definition/references) in dashboard or mineflayer-agent → `serena` MCP instead of grepping full files
...
- 2+ independent open items (e.g. farm-coord fix + the 3 open mineflayer security findings) → `superpowers:dispatching-parallel-agents` instead of serializing them
