...
// mineflayer-test's db.js has a direct enqueue(type, requestedBy, paramsJson,
// actions); this JobManager (Task 1) splits job creation into a planning
// stage instead (insertPlanningJob then attachPlan), so this test harness
// composes the two to reach the same queued-with-actions state the ported
// engine expects to run.
function enqueue(db, type, requestedBy, paramsJson, actions) {
  const jobId = db.insertPlanningJob(type, requestedBy);
  db.attachPlan(jobId, type, paramsJson, actions);
  return jobId;
}
...
test('full run: even break/place balance completes the job', async () => {
  ...
  const jobId = enqueue(db, 'flatten', 'player1', '{}', [
...
test('stuck loop: too many consecutive action failures aborts the job early instead of grinding through the rest', async () => {
  ...
  const jobId = enqueue(db, 'flatten', 'player1', '{}', actions);
